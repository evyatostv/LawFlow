...
export default function CalendarPage() {
  ...
  const { events, clients, cases, addEvent } = useAppData();
    ...
    <div className="flex flex-col gap-6">
          ...
          <div className="mt-3 space-y-3 text-sm">
            {events.map((eventItem) => (
              <div key={eventItem.id} className="flex items-center justify-between rounded-xl bg-white/70 p-3">
                ...
                <span className="text-xs text-steel/70">{eventItem.startAt}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="אירוע חדש">
        <form className="space-y-3" onSubmit={onSubmit}>
          ...
          <Input label="התחלה" value={form.startAt} onChange={(e) => setForm({ ...form, startAt: e.target.value })} />
          <Input label="סיום" value={form.endAt} onChange={(e) => setForm({ ...form, endAt: e.target.value })} />
